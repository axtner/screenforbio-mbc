...
exec > >(tee -a weighted_protax_classify.`date +%Y-%m-%d`.log)
exec 2> >(tee -a weighted_protax_classify.`date +%Y-%m-%d`.log >&2)

##### INFO

# weighted_protax_classify.sh
...
then
	echo ""
	echo "You are trying to use weighted_protax_classify.sh but have not provided enough information."
	echo "Please check the following:"
	echo ""
	echo "usage: bash weighted_protax_classify.sh pathToData locus protaxdir screenforbio outdir"
	echo "where:"
  ...
  echo "protaxdir is the path to a directory containing weighted protax models and clean databases for all 4 loci (one subdirectory per locus)"
	...
	echo "Welcome to weighted_protax_classify.sh: batch classification of FASTQ files with a weigthed PROTAX model"
	echo ""
...
##### END
echo "weighted_protax_classify complete for all input files"
end=`date +%s`
runtime=$((end-start))
...
echo "Classification for each read at each taxonomic level (species, genus, family, order) in files sample_name.w_level_probs"
echo "Headers are:"
echo "	queryID	taxID	log(probability)	level	taxon"
echo "Additionally, the best matching hit (for assigned species/genus where available) found with LAST is appended to sample_name.w_species_probs in sample_name.w_species_probs_sim"
echo "Headers are:"
